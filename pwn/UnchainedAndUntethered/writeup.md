# Unchained 
This problem is an unconstrained out-of-bounds (OOB) read and write on a global `uint32_t` array with no constraints on values allowed to be written or on the indexes that can be used. 
Players have to use the OOB read to leak different values in  memory so that they can bypass ASLR on the binary itself, `glibc`, and the stack before using the OOB write to gain code execution. 

## Solution
Indexing with the OOB read/write initially lets you read any location in the binary safely, as the offset from the OOB array `g_global_buffer` to anywhere within the binary won't change. The calculation to determine the correct index for any offset in the binary was `((offset_within_binary - g_global_buffer_offset) / sizeof(uint32_t))`. 

The OOB read/write can be turned into an arbitrary read/write by identifying the virtual address of `g_global_buffer`. OOB reading any location in the binary that contains a pointer to a location in the binary accomplishes this. I used the value at the symbol `__frame_dummy_init_array_entry` because it is contained a pointer to another symbol, `frame_dummy`. Subtracting the offset of `frame_dummy` in the binary from the read pointer at `__frame_dummy_init_array_entry` gives you the base address of the binary: `base_address = leaked_frame_dummy_address - frame_dummy_offset`.

Finding the base address of `glibc` can be done in the same way by OOB reading a pointer to a location in `glibc`, multiple of which can be found in entries of the global-offset-table (GOT). `setvbuf` was called at the start of the binary, so that entry in the GOT is populated. 
